...
range from approximately 0.24 (ellipsoidal/refill) to 0.53
(spherical/buoyancy), with the probability of energetic balance
$P(\mathrm{ratio} \geq 1)$ ranging from about 0.1\% to 21\% across models.
...
\tablehead{
\colhead{Reference} & \colhead{Reported $\Pcav$} & \colhead{Age} &
\colhead{Model} & \colhead{Our $\Pcav$}
}
\startdata
\citet{McDonald2015} &
  $1.0^{+1.5}_{-0.4} \times 10^{46}$ &
  Buoyancy & Sph/Buoy & $0.58 \times 10^{46}$ \\
\citet{Hlavacek-Larrondo2015} &
  $2$--$7 \times 10^{45}$ &
  Sound & Sph/Sound & $4.0 \times 10^{45}$ \\
\citet{McDonald2019} &
  ${\sim}10^{46}$ (review) &
  Mixed & All & $0.25$--$1.25 \times 10^{46}$ \\
\enddata
\tablecomments{Units are $\erg\;\s^{-1}$ throughout.}
\end{deluxetable}
